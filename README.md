...
4. Connect to the Battery Service device using an LE client (LEExplorer/Android) or (BLE Utility/Apple Store) or the WICED battery\_service\_client application.
5. Once connected the client can read Battery levels.
6. If the client enables the notification, the Battery Server will send the battery level every 30 secs.
