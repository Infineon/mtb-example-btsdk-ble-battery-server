...
4. Connect to Battery Service using one of the LE clients (LEExplorer/Android) or (BLE Utility/Apple Store) or battery\_service\_client application.
5. Once connected the client can read Battery levels.
6. If the client enables the notification, the Battery Server will send battery level every 30 secs.
7. Battery Level starts from 100 and keeps decrementing to 0. The Battery Server App is designed such that, once it hits 0, it will be rolled back to 100.
