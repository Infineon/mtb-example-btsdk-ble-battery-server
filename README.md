...
3. On application start the device acts as a GATT server and advertises itself with the Battery Service UUID.
4. Connect to the Battery Service device using an LE client (LEExplorer/Android) or (LE Utility/Apple Store) or the AIROC&#8482; battery\_service\_client application.
5. Once connected the client can read Battery levels.
6. If the client enables the notification, the Battery Server will send the battery level every 2 secs.
7. Battery Level starts from 100 and keeps decrementing to 1. The Battery Server App is designed such that, once it hits 1, it will roll over back to 100.
